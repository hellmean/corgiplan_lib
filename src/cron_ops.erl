...
% original code taken from https://github.com/antoniogarrote/cronparser.erl
% this version has been modified to remove some unreachable code and reduce time complexity of some functions.
...
-record(cron_entry,
        {minute_set, hour_set, dom_set, month_set, dow_set, minute, hour, dom, month, dow}).
...
parse_elements(ToParse, MinRange, MaxRange) ->
    ...
    Times =
        lists:map(fun(Element) -> parse_element(Element, MinRange, MaxRange) end, Elements),
    lists:flatten(Times).
        ...
        {match, _} ->
            ...
            Step =
                if Len =:= 1 ->
                       1;
                   Len =/= 1 ->
                       list_to_integer(string:substr(Element, 3, Len))
                end,
        ...
        _ ->
            case re:run(Element, ?SUBELEMENT_REGEX, [{capture, all, list}]) of
                ...
                {match, [_, StartRange, _, EndRange, _, Step]} ->
                    stepped_range(list_to_integer(StartRange),
                                  list_to_integer(EndRange),
...
stepped_range(StartRange, EndRange, Step) ->
    ...
    lists:map(fun(Iteration) -> StartRange + Step * Iteration end, Iterations).
...
time_specs(Source) ->
    ...
    MinuteVals =
        lists:sort(
            lists:flatten(parse_elements(Minute, 0, 59))),
    HourVals =
        lists:sort(
            lists:flatten(parse_elements(Hour, 0, 23))),
    DomVals =
        lists:sort(
            lists:flatten(parse_elements(Dom, 1, 31))),
    MonthVals =
        lists:sort(
            lists:flatten(parse_elements(Month, 1, 12))),
    DowVals =
        lists:sort(
            lists:flatten(parse_elements(Dow, 0, 6))),

    #cron_entry{minute_set = sets:from_list(MinuteVals),
...
find_best_next(Current, Allowed) ->
    Values = lists:filter(fun(X) -> X > Current end, Allowed),
    case Values of
        [] ->
            nil;
        _ ->
...
compute_next_time(Now, Spec) ->
    ...
    Seconds = 0,
    #cron_entry{minute = MinuteVals,
                hour = HourVals,
                hour_set = HoursSet} =
        Spec,
    [NextMinute, NextHour, NextDate] =
        case find_best_next(Minute, MinuteVals) of
            nil ->
                case find_best_next(Hour, HourVals) of
                    nil ->
...
compute_next_date(Now, Spec) ->
    ...
    #cron_entry{month = MonthVals,
                dom = DayVals,
                dow_set = DaysOfWeekSet,
                month_set = MonthsSet} =
        Spec,
    #cron_entry{hour = HourVals, minute = MinuteVals} = Spec,
    {NextDate, NextTime} =
        case find_best_next(Day, DayVals) of
            nil ->
                case find_best_next(Month, MonthVals) of
            ...
            DayVal ->
                case sets:is_element(Month, MonthsSet) of
                    ...
                    false ->
                        case find_best_next(Month, MonthVals) of
                            nil -> % first day, first month -> next year
                                {{Year + 1, hd(MonthVals), hd(DayVals)},
                                 {hd(HourVals), hd(MinuteVals), 0}};
                            MonthVal -> % first day -> next month
                                {{Year, MonthVal, hd(DayVals)}, {hd(HourVals), hd(MinuteVals), 0}}
                        end
                end
    ...
    try day_of_week(NextDate) of
        ...
        _ ->
            ...
            case find_best_next(ThisMonth, MonthVals) of
                nil ->
                    compute_next_date({{ThisYear + 1, hd(MonthVals), hd(DayVals)},
                                       {hd(HourVals), hd(MinuteVals), 0}},
                                      Spec);
                SafeMonth ->
                    compute_next_date({{ThisYear, SafeMonth, hd(DayVals)},
                                       {hd(HourVals), hd(MinuteVals), 0}},
                                      Spec)
            end
    end.
